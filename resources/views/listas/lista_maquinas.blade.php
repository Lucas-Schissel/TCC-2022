		...
		@foreach ($maq as $u)
		  <tr class="table-light" >
			...
			<td>
			 ...
			 <a class="btn btn-warning mt-1" href="{{ route('maquina_update', [ 'id' => $u->id ])}}"> 
			 Alterar
...
<div class= "row">
	<div class="navbar-expand-lg navbar navbar-dark bg-dark w-100">
		...
		<a class="btn btn-secondary m-1 p-1" type="button2" href="{{ route('maquina_cadastro') }}">
...
<script>
	$('.delete').on('click', function(){
		...
		var id = $(this).data('id'); 
		$('span.nome').text(nome); 
		$('a.delete-yes').attr('href', '/maquina/excluir/' +id); 
		$('#excluir').modal('show');
	});
</script>
